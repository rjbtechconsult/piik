...
interface CreateItemModalProps {
  onClose: () => void;
  onSave: (title: string, type: string, assigneeUniqueName: string, parentId?: number, areaPath?: string, status?: string, startDate?: string, endDate?: string) => Promise<void>;
...
export function CreateItemModal({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use parentItem.id if it's a sub-item, otherwise use selectedParentEpic if it's a Story (Epics have no parent)
      const effectiveParentId = type === "Epic" ? undefined : (parentItem?.id || (selectedParentEpic ? parseInt(selectedParentEpic) : undefined));
      await onSave(title, type, assignee, effectiveParentId, parentItem?.areaPath, status, startDate || undefined, endDate || undefined);
      onClose();
    } catch (error: any) {
        ...
        <div className="p-5 overflow-y-auto max-h-[85vh]">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-sm font-bold text-[var(--text-main)] uppercase tracking-widest">
              {parentItem ? "New Sub-Item" : type === "Epic" ? "New Epic" : "New Story"}
            </h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              >
                {parentItem ? (
                  ...
                  <>
                    <option value="User Story" className="bg-[var(--app-bg-solid)]">User Story</option>
                    <option value="Epic" className="bg-[var(--app-bg-solid)]">Epic</option>
                  </>
                )}
              </select>
            </div>

            {!parentItem && type !== "Epic" && (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[var(--text-dim)] uppercase tracking-wider ml-1">Start Date</label>
                <input
                  type="date"
                  value={startDate}
                  onChange={(e) => setStartDate(e.target.value)}
                  className="w-full bg-white/5 border border-[var(--border-main)] rounded-xl px-3 py-2.5 text-[12px] text-[var(--text-main)] focus:outline-none focus:border-[var(--accent-blue)]/50 focus:bg-white/[0.08] transition-all [color-scheme:dark]"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-[var(--text-dim)] uppercase tracking-wider ml-1">End Date</label>
                <input
                  type="date"
                  value={endDate}
            ...
            >
              {isSubmitting || isLoading ? (
                ...
                <>
                  ...
                  {parentItem ? "Create Sub Item" : type === "Epic" ? "Create Epic" : "Create Story"}
                </>
              )}
            </button>
          </form>
